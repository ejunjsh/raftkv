...
func TestStreamWriterAttachOutgoingConn(t *testing.T) {
	sw := startStreamWriter(testLogger, types.ID(0), types.ID(1), newPeerStatus(testLogger, types.ID(0), types.ID(1)), &fakeRaft{})
...
func TestStreamReaderDialRequest(t *testing.T) {
	for i, tt := range []streamType{streamTypeMessage, streamTypeMsgAppV2} {
		...
		if g := req.Header.Get("X-Cluster-ID"); g != "1" {
			t.Errorf("#%d: header X-Cluster-ID = %s, want 1", i, g)
		}
...
func TestStreamReaderStopOnDial(t *testing.T) {
	...
	sr := &streamReader{
		...
		status: newPeerStatus(testLogger, types.ID(1), types.ID(2)),
