...
func TestPipelineKeepSendingWhenPostError(t *testing.T) {
	...
	picker := mustNewURLPicker(t, []string{"http://localhost:2380"})
	tp := &Transport{pipelineRt: tr, Logger: testLogger}
	p := startTestPipeline(tp, picker)
	defer p.stop()
...
func TestPipelineExceedMaximumServing(t *testing.T) {
	...
	picker := mustNewURLPicker(t, []string{"http://localhost:2380"})
	tp := &Transport{pipelineRt: rt, Logger: testLogger}
	p := startTestPipeline(tp, picker)
	defer p.stop()
...
func TestPipelinePost(t *testing.T) {
	...
	picker := mustNewURLPicker(t, []string{"http://localhost:2380"})
	tp := &Transport{ClusterID: types.ID(1), pipelineRt: tr, Logger: testLogger}
	p := startTestPipeline(tp, picker)
	...
	//}
	if g := req.Header.Get("X-Cluster-ID"); g != "1" {
...
func TestPipelinePostBad(t *testing.T) {
	...
	for i, tt := range tests {
		...
		tp := &Transport{pipelineRt: newRespRoundTripper(tt.code, tt.err), Logger: testLogger}
		p := startTestPipeline(tp, picker)
...
func TestStopBlockedPipeline(t *testing.T) {
	picker := mustNewURLPicker(t, []string{"http://localhost:2380"})
	tp := &Transport{pipelineRt: newRoundTripperBlocker(), Logger: testLogger}
	p := startTestPipeline(tp, picker)
